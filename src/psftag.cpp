...
#include <string.h>
 ...
 /*
 ** Note: errno.h is often #included by default, but keep this reference for
 ** those compilers it's not automatically included in.
 */
#include <errno.h>
...
static int find_tag_var_start(const char *tagbuffer, const char *varname) {
  int i, j;
  static unsigned char commentSet;

  /*
  ** Modification by: ugetab(AKA, if I screwed up, don't yell at Neill!)
  ** If the varname is Comment, set commentSet.
  ** If commentSet, then return -1.
  */
    ...
    if(!varname[j]) {
      ...
      */
      for(;; j++) {
        ...
        if(u == '=') {
		if(commentSet == 1) {return -1;}
		/* varname was lowercased, so this will work with a constant case*/
		if(varname[0] == 99 && varname[1] == 111 && varname[2] == 109 && varname[3] == 109 && varname[4] == 101 && varname[5] == 110 && varname[6] == 116 && !varname[7]) {commentSet = 1;}
		return i;
	}
